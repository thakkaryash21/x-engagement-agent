...
**Findings shape (what to pass in `findings`).** Each finding is a dict with at least `text` plus the metadata fields the write-back schema owns — `scope` (`world` | `self`), `source_type`, `context_type`, `confidence`, `importance` (durability 0–1: a lasting lesson rates high, a passing detail low), `entities`, `provenance`, and a **`subject_slug` on *every* finding — world and self alike** (a topic slug such as `cruitical-hiring-thesis` or `workplace-chat`; do **not** leave it blank on self findings — blank ones all collapse into one `misc` dossier, defeating per-subject retrieval, `reflect()` consolidation, and human curation. A self-fact is *about* a subject the persona has lived experience with, so slug it by that subject — often the same slug as its paired world subject, giving a `subjects/<slug>.md` ↔ `self/<persona>/<slug>.md` pair) (plus **`persona` — required for every `scope=self` chunk** so self memory is isolated per persona and routed to `self/<persona>/`; `--persona <persona>` on the call stamps it defensively); include the raw `evidence` (the on-X snapshot captured browser-only, or the persona-history excerpt) so the immutable evidence record is written.

**Seed for breadth, and promote durable facts on the first pass.** Two rules follow from seeding passing **no `--reply-id`**, and both are load-bearing — the default reading ("let the gate decide") produces a thin seed:

- **Mark durable facts `reusable: true`.** With no `--reply-id`, a finding that doesn't promote is **not** kept as a brief (unlike `scroll` §2.4b) — it survives only as raw evidence and never reaches the retrievable store. And the sighting-count promotion path needs a *second* encounter, which a one-shot seed does not provide. So set `reusable: true` on every lasting fact — each shipped project, stated opinion/worldview, outcome, documented relationship (Self), and each subject the persona **recurrently** engages (World) — so it promotes on this first sighting. Leave `reusable` off only for genuinely passing details; the persona's own posts are curated history, so most seed facts *are* durable.
- **Emit one finding per distinct fact — aim for dozens, not a handful.** Do not collapse a session into a few summary findings. A founder's sampled history yields many durable Self facts (distinct projects, opinions, lessons, relationships) and several recurring World subjects — emit a **separate** finding for each. A healthy first seed over a real sample is dozens of findings; a single-digit result means the pass summarized instead of enumerating. Density then compounds as `scroll`/`review` and later seed passes add sightings and merge.
...
1. **Sampling + budget.** Process a **bounded sample per session** — cap by a tweet count or the same enrichment budgets `scroll` uses: `max_context_lookups_per_session` and the research-tab wall-clock sub-budget `context_research_time_budget_minutes` (`config/limits.yaml`). Prioritize **high-signal history** (own posts, high-engagement replies) over exhaustive coverage; a `sampled_seed` scope (§0.1) is the norm here, not `full_archive`.
...
```powershell
python -m dashboard.context_cli reflect --scope self  --subject <slug> --persona <persona>   # per self subject the batch touched (self reflection is BOTH per-persona and per-subject)
python -m dashboard.context_cli reflect --scope world --subject <slug>                        # per recurring world subject the batch enriched
```

Seeding *gathers* raw chunks; reflection collects them into one labeled `insight` chunk per scope/subject (a subject's consolidated current state; the persona's recurring stance across scattered opinion chunks). **Interim behaviour (planned/not-yet-executable synthesis):** today this is a *simple concatenating consolidation* — the `insight` text is a labeled join of the source chunks' texts, not a distilled higher-level abstraction. Real synthesis (an insight beyond the union of the source texts) is a deferred roadmap feature (`docs/roadmap.md` → Deferred); the chunk is honestly labeled `context_type=insight` so consumers can tell interim consolidation from the future pass. Run it **per subject** the batch touched, for **both** scopes (self also passes `--persona`). Do **not** omit `--subject` to consolidate a whole scope — a scope-wide reflect concatenates every subject into one giant insight that then outranks the specific chunk it contains at retrieval time, and collapses into a single slug-less dossier. Per-subject insights stay focused and retrievable. Reflections flow through the same dedup/merge, so running this every batch refreshes the one insight chunk per subject instead of appending duplicates.
