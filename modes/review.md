...
- the context memory subsystem, through the policy layer `ContextMemory` (`dashboard/context_memory.py`, over `ContextStore`) only — `write_back` of sent-reply world/self chunks and `reflect` after ingesting metrics (§6); never by editing chunk/index files directly
- a voice-authenticity dashboard — the ensemble read of §7 (three signals side by side, not one score)
...
The context memory subsystem is reached **only** through the stable policy layer `ContextMemory` (`dashboard/context_memory.py`), which sits over the swappable substrate `ContextStore`. `review` uses `ContextMemory.write_back(findings)` and `ContextMemory.reflect(scope, subject?)` in §6 and `ContextMemory.style_exemplars(persona, k)` in §7. It never reads or writes the chunk/evidence/index files under `data/context/` directly — the policy layer owns distill, dedup/merge, the promotion gate, evidence preservation, the retrieval score blend, and reflection.
...
- **`gap_type` (none | local | discourse | factual | mixed)** — which *kind* of resolved gap correlates with lift (does discourse research beat factual lookup for a given format, or the reverse).
- **`source_types` (subset of `dossier,x_navigation,x_search,web_search`)** — which adapter routes pull weight. A route that never correlates with lift is a candidate for a tighter budget.
...
Step 5 promotes high-confidence *voice* rules into the style docs. Symmetrically, `review` grows the *context* memory subsystem over time — through the same unified write-back the drafting and `learn` modes use (`ContextMemory.write_back`, the policy layer over `ContextStore`). A sent reply is both a documented persona utterance and a fresh observation about the subjects it touched, so the post-send data is legitimate new memory. This runs only for items captured this session, and only from what the sent text and reviewed metrics actually evidence — **never invent** a fact or an experience (AGENTS.md §6). The no-invention contract is what makes this safe: every chunk written here traces to the sent reply or the reviewed source.

For each reviewed sent item, distill and write back in both scopes plus the content dimension:

- **World** — a durable fact about a recurring subject that this review confirmed (a positioning that held, a claim that proved out, a subject's consolidated current state) → a `scope=world` finding, so the next reply about that subject hits memory instead of re-spending a lookup. The write-back's promotion gate keeps only the *reusable* subset; a one-off detail stays out of durable memory.
- **Self** — any self-fact the sent reply publicly expressed (an opinion the persona took, a project outcome referenced, a relationship shown) → a `scope=self` finding (`source_type=persona_history`, `confidence` per how explicit the statement was, the sent reply text preserved as its evidence). The persona's live posting continuously grows the Self well, not just the one-time `learn` seeding — and because the chunk traces to the actual sent reply, it satisfies the documented-source rule by construction.
...
Each finding is a `ContextMemory.write_back` finding dict (`text` plus `scope` / `source_type` / `context_type` / `confidence` / `importance` / `entities` / `provenance` / `evidence`, and an optional explicit `reusable` verdict). The policy layer handles distill → dedup/merge → promotion gate → store and preserves the raw evidence immutably — `review` does not touch chunk files. Re-reviewing the same item merges (bumps `last_refreshed`) rather than duplicating, because it flows through the same dedup.

**Reflection after ingest.** Once this session's metrics and write-backs are in, call `ContextMemory.reflect(scope, subject?)` over the subjects and the persona this session touched — the review-side trigger for the consolidation layer, symmetric with `learn`'s per-batch reflection. Reflection synthesizes the accumulated post-send observations into refreshed `insight` chunks (an updated consolidated state for a subject that kept recurring; a sharpened persona stance from many scattered opinion chunks) — a higher abstraction no single raw chunk holds, distinct from the janitorial dedup the write-back already did. Run it as a batch pass at end of session, never on a hot drafting path. Reflection flows through the same dedup, so running it every `review` refreshes the one insight chunk instead of appending a duplicate each time.

## 7. Voice-authenticity ensemble (a dashboard, not one score)

Beyond engagement, `review` assesses the *voice authenticity* of this session's sent drafts — but with an **ensemble of complementary signals reported as a small dashboard, never collapsed into a single number**. No single authenticity metric is trustworthy alone, and a blended score would erase the divergences between the signals, which are the actual finding. Compute, over the sent drafts:

- **Authorship-attribution** — does the sent text read as *this persona* versus a generic/AI voice? Score each sent draft against the persona's own documented voice: the style doc plus the persona's own `scope=self` exemplars from `ContextMemory.style_exemplars(persona, k)` (selected for stylistic *diversity/coverage*, explicitly not topical similarity — topic-matched exemplars degrade the comparison).
- **NLI self-consistency** — how often sent drafts drift off-persona: the aggregate of the per-draft natural-language-inference persona-consistency judgment (entailment/neutral = OK, contradiction = drift) over the persona's documented self-lines. This is the batch aggregate of the same signal the drafting gate applies pre-send.
- **AI-detector probe** — a weak tell-tracker only, tracked as a *trend*, not a target to game: indistinguishability is not the goal, and the human send-gate is the backstop (AGENTS.md §1). A rising probe score is a hint to inspect the anti-AI passes, nothing more.

Report the three side by side. **A divergence between them is itself the finding** — high engagement but a rising AI-detector score, or strong attribution but climbing NLI contradiction — and that divergence is what gets written to `data/learnings/` (a `content-playbook.md` observation or an `engagement-targets.md` entry) for the drafting passes to act on. Do not average them into one score. Voice outranks metrics (AGENTS.md §6): an authenticity regression is worth acting on even when engagement is up.
