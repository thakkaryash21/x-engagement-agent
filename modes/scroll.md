...
1. **Gap analysis.** Reading the tweet + local thread, ask what you'd need to know to reply *specifically* that you don't already know, and emit the structured gap set the taxonomy defines (`guidelines/context-enrichment.md` §2, §4 — extract the named entities/terms/claims first, then `[{context_type, entity/claim, why_needed}]`). No gaps → skip enrichment, proceed to 2.5 with an empty brief (`context_used=none`).
