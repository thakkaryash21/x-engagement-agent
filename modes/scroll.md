...
1. **Gap analysis.** Reading the tweet + local thread, ask what you'd need to know to reply *specifically* that you don't already know, and emit the structured gap set the taxonomy defines (`guidelines/context-enrichment.md` §2, §4.1 — extract the named entities/terms/claims first, then `[{context_type, entity/claim, why_needed}]`). No gaps → skip enrichment, proceed to 2.5 with an empty brief (`context_used=none`).
...
4. **Gather.** Execute: memory (0, free) — retrieve from **both scopes** via the context CLI (the runtime bridge over `ContextMemory`; construct queries per the archetype-gated blend, self-lean archetypes pull the persona's relevant documented experience, world-lean pull subject facts):
   ```powershell
   python -m dashboard.context_cli search --query "<gap query>" --scope <world|self|both> --gap-type <identity|factual|temporal|discourse|relational>
   ```
   It prints JSON: the ranked, recency-blended top-k chunks. Then → in-tab (a, cheap) → routed (b)/(c) for the residual *world* gap. Self is never gathered live from the open web — it comes only from memory seeded by `learn` and user input. Adapter (b) uses the scoped serial research tab (`guidelines/context-enrichment.md` §6 → AGENTS.md §3.4).
5. **Grade context sufficiency + escalate.** Run the graded sufficiency gate owned by `guidelines/context-enrichment.md` §4.7: grade the accumulated context **good / insufficient / ambiguous**, escalate one source rung (memory → on-X → off-X web) on insufficient/ambiguous, treat a *stale* memory hit as a reason to escalate to live search, and stop on *good*, on budget hit, or after two unproductive refinements. The mode-specific inputs/outputs: the budget cap is `max_context_lookups_per_session` (`config/limits.yaml`), and a stop-without-*good* means marking the residual `[VERIFY: ...]` or **skipping** the candidate — never draft a generic caveat.
