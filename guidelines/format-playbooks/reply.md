...
- `reply_rank` (recorded in `data/csv/replies.csv`) matters: an early reply (low rank) earns more visibility, so favor target tweets that are still early per `target-posts.md`.
