...
`compose` mode drafts original tweets (not replies). This file defines how a topic and a hook get selected before writing — both are chosen explicitly and recorded in `data/data/tweets.csv` (`content_type`, `hook_type`) so the review loop can attribute performance to them.
...
Same as `modes/scroll.md` §2.6: persona voice guide → style doc → Anti-AI Bible final pass (hard gate). Append to `data/data/tweets.csv` (`status=drafted`), write the draft file per the template in `modes/compose.md` §3 (target/author lines omitted for original tweets).
...
<!-- review mode: once >=5 reviewed sends exist, summarize which content_type/topic
     combinations correlate with the optimize targets in data/config/metrics.yaml for the
