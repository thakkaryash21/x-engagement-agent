...
- **Storage (files are the system):** under gitignored `data/context/` — Layer-1 dossiers (`subjects/<slug>.md`, `self/<persona>/<slug>.md`, human-editable), Layer-2 per-draft briefs (`briefs/<reply_id>.md`), immutable `evidence/`, flat `chunks/`, and a derived, rebuildable vector `index/`. Layer-3 analytics: `csv/context-provenance.csv` (schema in `dashboard/tables.py`).
- **Code seam:** `dashboard/context_store.py` (`ContextStore` — the swappable LanceDB + fastembed substrate: `add`/`search`/`reindex`, no policy) beneath `dashboard/context_memory.py` (`ContextMemory` — the stable policy: write-back / dedup / promotion / reflect / retrieval blend / style-exemplars). The modes call it through the CLI **`python -m dashboard.context_cli`** (subcommands `search` / `write-back` / `reflect` / `style-exemplars` / `reindex`) — no running-server dependency.
- **Enable:** `config/app.yaml → agent_web_search: true` (the launcher appends `--search`).
- **Deferred (documented, not executable yet):** real `reflect()` synthesis and the `review` voice-authenticity ensemble — see [docs/roadmap.md](docs/roadmap.md). The prose labels them planned; a session runs neither today.
